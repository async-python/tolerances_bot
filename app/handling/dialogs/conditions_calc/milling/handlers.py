"""Conditions milling dialog handlers."""
...
from app.handling.states.conditions_calc import ConditionsMillingSG, ConditionsStartSG
...
async def _process_milling_conditions(
    ...
    """Receives, process and saves milling conditions."""
...
async def on_diameter_received(
    ...
    """Calculate milling conditions if tool diameter changed."""
...
async def on_cutting_speed_received(
    ...
    """Calculate milling conditions if cutting speed changed."""
    await _process_milling_conditions(
        message=message,
...
async def on_spindle_speed_received(
    ...
    """Calculate milling conditions if spindle speed changed."""
    await _process_milling_conditions(
        message=message,
...
@inject_resources
async def on_teeth_number_received(
    message: Message,
    message_input: MessageInput,
    dialog_manager: DialogManager,
    i18n: "TranslatorRunner",
    ...
    """Calculate milling conditions if number of teeth changed."""
    await _process_milling_conditions(
        message=message,
        value_type=ValueType.NUMBER_OF_TEETH,
        dialog_manager=dialog_manager,
        i18n=i18n,
    )
    await dialog_manager.next()
...
@inject_resources
async def on_feed_per_tooth_received(
    message: Message,
    message_input: MessageInput,
    dialog_manager: DialogManager,
    i18n: "TranslatorRunner",
    ...
    """Calculate milling conditions if feed per tooth changed."""
    await _process_milling_conditions(
        message=message,
